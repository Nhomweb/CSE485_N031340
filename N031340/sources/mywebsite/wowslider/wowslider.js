...
jQuery.fn.wowSlider=function(D) {
    ...
    D=L.extend( {
        ...
        , prev:"", next:"", duration:1000, delay:20*100, captionDuration:1000, captionEffect:0, width:960, height:360, thumbRate:1, caption:true, controls:true, autoPlay:true, stopOnHover:0, preventCopy:1
    }
    , D);
    var a=L(".ws_images", l);
    var Q=a.find("ul");
    ...
    if(U) {
        U.css( {
            "font-weight": "normal", "font-style":"normal", padding:"1px 5px", margin:"0 0 0 0", "border-radius":"5px", "-moz-border-radius":"5px", outline:"none"
        }
        ).attr( {
        ...
        if(n.length) {
            ...
            if(al.length) {
                ...
                function aq(aA) {
                    ...
                    am.show();
                    var aB= {
                        left: au.get(aA).offsetLeft-am.width()/2, "margin-top":au.get(aA).offsetTop-au.get(0).offsetTop+"px", "margin-bottom":-au.get(aA).offsetTop+au.get(au.length-1).offsetTop+"px"
                    }
                    ;
                    var az=al.get(aA);
                    var ay= {
        ...
        L("A", S).each(function(am) {
            if(am==c) {
                ...
                if(!M) {
                    ...
                    ;
                    f.stop(true).animate( {
                        left: -Math.max(Math.min(al.left, -an.left), al.left+ak.width()-S.width()), top:-Math.max(Math.min(al.top, -an.top), al.top+ak.height()-S.height())
                    }
                    )
                }
            }
    ...
    function o(am, aq, c, ao, ak) {
        ...
        L(al[0]).css( {
            left: (aq.left1||0)+"px", top:(aq.top1||0)+"px"
        }
        );
        L(al[1]).css( {
            left: (aq.left2||0)+"px", top:(aq.top2||0)+"px"
        }
        );
        ...
        function an(at) {
            ...
            , {
                duration: ak, easing:(aq.easing||ao), queue:false
            }
            )
        }
        an(0);
    ...
    function X(ap, at) {
        ...
        ap.show();
        var ao= {
            width: A(ap.get(0), 1, 1), height:v(ap.get(0), 1, 1), "float":ap.css("float"), overflow:"hidden", opacity:0
        }
        ;
        for(var ak in ar) {
        ...
        if(ap.css("position")=="static") {
            ...
            L.extend(ao, {
                position: ap.css("position"), zIndex:ap.css("z-index")
            }
            );
            ap.css( {
        ...
        , {
            duration: at.duration, easing:at.easing
        }
        );
        ap.animate(an, {
            queue:false, duration:at.duration, easing:at.easing, complete:function() {
                ap.css(ar);
    ...
    if(K) {
        if(window.Audio&&K.canPlayType&&K.canPlayType("audio/mp3")) {
            ...
            ;
            swfobject.createSWF( {
                data: V+"player_mp3_js.swf", width:"1", height:"1"
            }
            , {
                allowScriptAccess: "always", loop:true, FlashVars:"listener="+s+"&loop=1&autoplay="+(D.autoPlay?1:0)+"&mp3="+K
            }
            , m);
            K=0
        }
